...
export function setup(googleApi) {
  drive = google.drive({ version: 'v3', auth: googleAuth(googleApi) });
}

function ensureGoogleDriveInitialized() {
  return drive !== undefined ? false : new GoogleDriveInitializedError();
}
/**
...
export async function upload(req, fieldName, options) {
  const googleIsNotInitializedError = ensureGoogleDriveInitialized();
  if (googleIsNotInitializedError) {
    return Promise.reject(googleIsNotInitializedError);
  }
