 ...
 * @param {string} path
 * @param {string|object} [options='utf8']
 ...
 * @returns {Promise}
 */
export function writeFile(path, content, options = 'utf8') {
  return new Promise((resolve, reject) => {
    fs.writeFile(path, content, options, err => (err ? reject(err) : resolve()));
  });
}
...
export function readAccess(path) {
  return new Promise((resolve) => {
    fs.access(path, fs.R_OK, (err) => {
      if (err) {
        logger.verbose(`File ${path} does not exist, going ahead without it`);
        resolve(false);
        return;
      }
