 ...
 * @param {stream.Readable|Buffer} body
 * @param {function(GoogleDriveFileResource)} resolve
 * @param {function(Error)} reject
 */
function uploadToGoogleDrive(parentDirectoryId, fileName, body, resolve, reject) {
  drive.files.create({
    resource: {
      name: fileName,
 ...
 */
export async function uploadFileFromHttpRequest(req, fieldName, options) {
  ...
  return new Promise((resolve, reject) => {
    ...
    form.on('part', (part) => {
      ...
      if (part.filename && part.name === fieldName) {
        ...
        uploadToGoogleDrive(parentDirectoryId, part.filename, part, resolve, reject);
      }
 ...
 * @param {object} options
 * @param {string} options.folder - directory to upload to
 * @param {string} options.fileName - name for uploading file
 * @returns {Promise.<GoogleDriveFileResource, Error>}
 */
export async function upload(body, options) {
  ensureGoogleDriveInitialized();

  const parentDirectoryId = await getFolderId(options.folder);

  return new Promise((resolve, reject) => {
    uploadToGoogleDrive(parentDirectoryId, options.fileName, body, resolve, reject);
  });
}
